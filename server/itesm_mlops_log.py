...
file_handler = logging.FileHandler('itesm_mlops.log') # Indicamos el nombre del archivo
file_handler.setFormatter(formatter) # Configuramos el formato
logger.addHandler(file_handler) # Agregamos el archivo
#logging.basicConfig(filename='itesm_mlops.log', level=logging.DEBUG,
...
if __name__ == "__main__":
    ...
    logging.debug(f'test roc-auc : {roc_auc_score(y_test, proba_pred)}')
    print(f'test accuracy: {accuracy_score(y_test, class_pred)}')
    logging.debug(f'test accuracy: {accuracy_score(y_test, class_pred)}')
